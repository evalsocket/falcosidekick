...
func (c *Client) EventHubPost(falcopayload types.FalcoPayload) {
	...
	log.Info("[INFO] : %v EventHub - Try sending event", c.OutputType)
	...
	if err != nil {
		...
		log.Info("[ERROR] : %v EventHub - %v\n", c.OutputType, err.Error())
		return
	}

	log.Info("[INFO]  : %v EventHub - Hub client created\n", c.OutputType)

	data, err := json.Marshal(falcopayload)
	if err != nil {
		c.setEventHubErrorMetrics()
		log.Info("[ERROR] : Cannot marshal payload: %v", err.Error())
		return
	}
	...
	log.Info("[INFO] : %v EventHub - Publish OK", c.OutputType)
}
